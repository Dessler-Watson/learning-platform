...
function Station({ index, question, activeIndex, z, phase }: { index: number; question: GameQuestion; activeIndex: number; z: number; phase: string }) {
  ...
  useFrame(() => { if (!isActive || triggered.current || (phase !== 'playing' && phase !== 'question')) return; const rb = characterRigidBody.current; if (!rb) return; const pz = rb.translation().z; if (Math.abs(pz - z) > 1.8) return; triggered.current = true; const side: DoorChoice = rb.translation().x < 0 ? 'A' : 'B'; const store = useGameStore.getState(); store.setPhase('question'); store.submitAnswer(side); const correct = side === question.correctAnswer; store.setPhase(correct ? 'correctFeedback' : 'incorrectFeedback'); if (!correct) store.setExplanation(question.explanation); });
  return (
